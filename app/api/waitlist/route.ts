...
import { z } from "zod";
import { dispatchWaitlistWelcome } from "@/lib/renderTransactionalEmail";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const insertResult = await supabase
      ...
      .select("id,email_delivery_status,email_delivery_attempts")
      .single();
    ...
    if (insertResult.error) {
      if (isDuplicateError(insertResult.error)) {
        ...
        if (existingResult.error || !existingResult.data) {
          console.error("[waitlist] DUPLICATE_LOOKUP_FAILED", {
            targetProjectRef,
            ...safeSupabaseError(existingResult.error),
          });
          return NextResponse.json({ status: "duplicate", emailStatus: "unknown" });
        }
        signup = existingResult.data;
      } else {
        const safe = safeSupabaseError(insertResult.error);
    ...
    if (!signup) {
      return NextResponse.json(
        { status: "error", code: "MISSING_INSERT_RESULT", message: "Unable to save waitlist signup right now." },
        { status: 500 },
      );
    }

    const shouldDispatch =
      signup.email_delivery_status === "pending" ||
      signup.email_delivery_status === "failed_retryable";
    let emailStatus = signup.email_delivery_status;

    if (shouldDispatch) {
      const delivery = await dispatchWaitlistWelcome({
      ...
      const deliveryUpdate: Record<string, unknown> = {
        ...
        email_delivery_attempts: (signup.email_delivery_attempts ?? 0) + 1,
        email_last_attempt_at: new Date().toISOString(),
        email_last_error_code: delivery.status === "sent" ? null : delivery.code,
      };
      if (delivery.status === "sent") deliveryUpdate.email_sent_at = new Date().toISOString();
      const { error: deliveryUpdateError } = await supabase
        .from(WAITLIST_TABLE)
        .update(deliveryUpdate)
        .eq("id", signup.id);
      if (deliveryUpdateError) {
        console.error("[waitlist] EMAIL_STATUS_UPDATE_FAILED", {
          targetProjectRef,
          code: safeSupabaseError(deliveryUpdateError).code,
        });
      }
    }
    ...
    return NextResponse.json(
      { status: isNewSignup ? "success" : "duplicate", emailStatus },
      { status: isNewSignup ? 201 : 200 },
    );
  } catch (err) {
